...
module MTLibcouchbase
    class Bucket
        ...
        #   operation will return a +MTLibcouchbase::Result+, otherwise (by default)
        ...
        #   results.
        #
        # @return [Object, Array, Hash, MTLibcouchbase::Result] the value(s)
        #
        # @raise [MTLibcouchbase::Error::KeyExists] if the key already exists on the server
        ...
        # @raise [MTLibcouchbase::Error::KeyNotFound] if the key doesn't exists
        #
        ...
        #   c.get("missing-foo", quiet: false)  #=> raises MTLibcouchbase::Error::NotFound
        #
        ...
        #
        # @return [MTLibcouchbase::Result] this includes the CAS value of the object.
        #
        # @raise [MTLibcouchbase::Error::KeyExists] if the key already exists on the server
        # @raise [Libouchbase::Error::Timedout] if timeout interval for observe exceeds
        # @raise [Libouchbase::Error::NetworkError] if there was a communication issue
        ...
        # @raise [Libouchbase::Error::NetworkError] if there was a communication issue
        # @raise [MTLibcouchbase::Error::KeyNotFound] if the key doesn't exists
        # @raise [MTLibcouchbase::Error::DeltaBadval] if the key contains non-numeric value
        #
        ...
        #   c.delete("foo", quiet: false)   #=> will raise MTLibcouchbase::Error::KeyNotFound
        #
        # @example Delete the key with version check
        #   res = c.set("foo", "bar")       #=> #<struct MTLibcouchbase::Response callback=:callback_set, key="foo", cas=1975457268957184, value="bar", metadata={:flags=>0}>
        #   c.delete("foo", cas: 123456)    #=> will raise MTLibcouchbase::Error::KeyExists
        ...
        #
        # @raise [MTLibcouchbase::Error::HttpError] in case of an error is
        #   encountered.
        #
        # @return [MTLibcouchbase::Response]
        #
        ...
        #
        # @yieldparam [MTLibcouchbase::SubdocRequest] the subdocument request object used to define the request
        #
        ...
        #
        # @return [MTLibcouchbase::DesignDocs]
        ...
        # {MTLibcouchbase::Error::KeyExists}.
        #
        # Setting the +:retry+ option to a positive number will cause this method
        # to rescue the {MTLibcouchbase::Error::KeyExists} error that happens when
        ...
        #
        # @return [MTLibcouchbase::Response] the transaction details including the new CAS
            ...
            connecting.synchronize {
                Thread.new do
                    @connection.reactor.run do |reactor|
                        ...
                        rescue MTLibcouchbase::Error::ConnectError => e
                            attempt += 1
                            if attempt < 3
                                reactor.sleep 200
